...
impl<'ctx> IRGenerator<'ctx> {
    ...
    pub fn generate_with_stdlib(
        ...
        if stdlib_stmts.is_empty() {
            *self.program_scope.borrow_mut() = Some(scope);
            let all_stmts: Vec<Rc<RefCell<Statement>>> = program.statements.to_vec();
        ...
        *self.module_name.borrow_mut() = saved_mod;
        ...
        for (class_name, _) in &vtable_globals_snapshot {
            ...
            if self.module.get_global(&vtable_global_name).is_none()
                && let Some(t) = self.vtable_types.borrow().get(class_name).copied()
            {
                let gv = self
                    .module
        ...
        *self.program_scope.borrow_mut() = Some(scope);
        let all_main: Vec<Rc<RefCell<Statement>>> = program.statements.to_vec();
        ...
        for field in properties.iter() {
                ...
                } = &*field_decl.borrow()
            {
                let has_get_set = accessors
                    .iter()
                    .any(|a| matches!(a.kind, AccessorKind::Get | AccessorKind::Set));
                if !has_get_set && let Ok(llvm_ty) = self.resolve_type(ty.clone()) {
                ...
                if field_name == param_name {
                    ...
                    if is_class {
                        if let Some(class_type) =
                            ...
                            && let Some(param_ptr) = self.lookup_variable(&param_name)
                            && let Ok(llvm_ty) = self.resolve_type(field_ty.clone())
                            && let Ok(param_val) = self.builder.build_load(llvm_ty, param_ptr, "")
                        {
    ...
    fn build_optional_some_value(
        ...
        let tag_ptr = self
            .builder
            .build_struct_gep(optional_llvm_ty, alloca, 0, "")?;
        self.builder
            .build_store(tag_ptr, self.context.i8_type().const_int(1, false))?;
        let payload_ptr = self
            .builder
            .build_struct_gep(optional_llvm_ty, alloca, 1, "")?;
        let some_slot_ptr = self
            .builder
            .build_struct_gep(payloads_type, payload_ptr, 1, "")?;
        let inner_ptr = self
            .builder
            .build_struct_gep(some_payload_type, some_slot_ptr, 0, "")?;
        ...
        match val {
            BasicValueEnum::IntValue(i) => {
                ...
                if i.get_type().get_bit_width() == 64 {
                    ...
                    Ok(self.builder.build_int_s_extend(i, i64_ty, "")?)
                }
            }
            BasicValueEnum::PointerValue(p) => {
                Ok(self
                    .builder
                    .build_ptr_to_int(p, self.context.i64_type(), "")?)
            }
            ...
            _ => anyhow::bail!("Cannot convert value to i64 for comparison"),
        }
    }
    ...
    fn get_enum_case_index(&self, enum_name: &str, case_name: &str) -> Result<usize> {
        let symbol_opt = self.program_scope.borrow().as_ref().and_then(|scope| {
            scope.borrow().get_symbol_deep(enum_name)
        }).or_else(|| self.find_symbol_in_packages(enum_name));
        if let Some(symbol) = symbol_opt
    ...
    fn get_enum_raw_llvm_type(&self, enum_name: &str) -> Option<BasicTypeEnum<'ctx>> {
        ...
        if let Some(symbol) = symbol_opt
            && let Symbol::Enum { decl, .. } = &*symbol.borrow()
            && let Statement::EnumDecl { raw_value_type, .. } = &*decl.borrow()
            && let Some(raw_type) = raw_value_type
        {
    ...
    fn compute_vtable_method_list(&self, class_name: &str) -> Vec<(String, String)> {
        ...
        let Some(symbol) = scope_ref
            .borrow()
            .get_symbol_qualified(class_name)
            .or_else(|| scope_ref.borrow().get_symbol_deep(class_name))
        else {
            return vec![];
        };
        let sym_borrow = symbol.borrow();
    ...
    fn ensure_vtable_type(&self, class_name: &str) -> StructType<'ctx> {
        if let Some(&vt) = self.vtable_types.borrow().get(class_name) {
            return vt;
        }
        if !self.class_has_vtable_methods(class_name) {
            let vtable_name = Self::mangle_type_name(
            ...
            let vtable_type = self.context.opaque_struct_type(&vtable_name);
            vtable_type.set_body(&[], false);
            self.vtable_types
                .borrow_mut()
                .insert(class_name.to_string(), vtable_type);
            return vtable_type;
        }
        let method_list = self.compute_vtable_method_list(class_name);
        let ptr_ty = self.context.ptr_type(inkwell::AddressSpace::from(0));
    ...
    fn build_vtable_method_fn_type(
        &self,
        class_name: &str,
        method_name: &str,
        ...
        let Some(symbol) = scope_ref
            ...
            anyhow::bail!("Class '{}' not found in scope", class_name);
        };
        let sym_borrow = symbol.borrow();
        let Symbol::Class { methods, .. } = &*sym_borrow else {
            anyhow::bail!("'{}' is not a class", class_name);
        };
        for m in methods {
            let mb = m.borrow();
            if mb.name().as_deref().ok() != Some(method_name) {
                continue;
            }
            let Ok(Some(decl)) = mb.get_decl() else {
                continue;
            };
            if let Statement::FunctionDecl {
                ty: Some(ty),
            ...
            } = &*decl.borrow()
            {
                if let Type::Function(param_types, return_type, is_vararg, throws_types) =
                    &*ty.borrow()
                {
                    let mut all_params: Vec<Rc<RefCell<Type>>> = Vec::new();
                    if !static_method {
                        all_params.push(Rc::new(RefCell::new(Type::Pointer(Rc::new(
                            RefCell::new(Type::Void),
                        )))));
                    }
                    if throws_types.is_some() {
                    ...
                    all_params.extend(param_types.iter().cloned());
                    return self.get_function_type(return_type.clone(), all_params, *is_vararg);
                }
            }
        }
        anyhow::bail!(
    ...
    fn build_property_accessor_fn_type(
        &self,
        class_name: &str,
        field_name: &str,
        is_setter: bool,
        ...
        let symbol = scope_ref
            ...
            .get_symbol_qualified(class_name)
            .or_else(|| scope_ref.borrow().get_symbol_deep(class_name))
            .ok_or_else(|| anyhow::anyhow!("Class '{}' not found in scope", class_name))?;
        let sym_borrow = symbol.borrow();
        let properties = match &*sym_borrow {
            Symbol::Class { properties, .. } => properties.clone(),
            Symbol::Struct { properties, .. } => properties.clone(),
            _ => anyhow::bail!("'{}' is not a class or struct", class_name),
        };
        drop(sym_borrow);
        for prop in &properties {
            let pb = prop.borrow();
            if pb.name().as_deref().ok() != Some(field_name) {
                continue;
            }
            let Some(decl) = pb.get_decl().ok().flatten() else {
                continue;
            };
            if let Statement::VariableDecl { ty: Some(ty), .. } = &*decl.borrow() {
                ...
                if is_setter {
                    return self.get_function_type(
                        ...
                        vec![self_param, ty.clone()],
                        false,
                    );
                } else {
                    return self.get_function_type(ty.clone(), vec![self_param], false);
                }
            }
        }
        anyhow::bail!(
    ...
    fn normalize_class_name<'a>(&self, name: &'a str) -> std::borrow::Cow<'a, str> {
        if name.contains('.') {
            if let Some(pos) = name.rfind('.') {
                return std::borrow::Cow::Owned(name[pos + 1..].to_string());
            }
        }
        ...
        if let Some((_proto_params, proto_param_tys)) = proto_params {
            for method in &methods {
                ...
                if let Statement::FunctionDecl { parameters, ty, .. } = &*decl_ref {
                    ...
                    let fn_borrow = fn_ty.borrow();
                    if let Type::Function(param_tys, _, _, None) = &*fn_borrow
                        && param_tys.len() == proto_param_tys.len()
                        && param_tys
                            .iter()
        ...
        match &*statement.borrow() {
            ...
            Statement::While {
                ...
                } = &*condition.borrow()
                {
                    ...
                    let enum_name = if let Some(t) = enum_type.as_ref() {
                        t.value.clone()
                        ...
                        self.get_enum_name_from_expr_string(expression.clone())
                    {
                            ...
                            && let Type::Enum(name, _, _) = &*ty.borrow()
                        {
                            ...
                            anyhow::bail!("while-case: cannot infer enum type")
                        }
                    };
                    ...
                    self.builder
                        .build_conditional_branch(match_result, body_bb, exit_bb)?;
                    ...
                    if !bindings.is_empty() {
                        ...
                        if let Some(payload_type) = enum_payloads.get(&enum_name) {
                            ...
                            let case_payload_ty = payload_type
                                ...
                                .ok_or_else(|| {
                                    anyhow::anyhow!("Case payload not found at {}", case_idx)
                                })?
                                .into_struct_type();
                            for (i, binding) in bindings.iter().enumerate() {
                                let token = match binding {
                                    Pattern::Identifier(t) => Some(t.as_ref()),
                                    Pattern::ValueBinding(inner) => {
                                        if let Pattern::Identifier(t) = inner.as_ref() {
                                            Some(t.as_ref())
                                        } else {
                                            None
                                        }
                                ...
                                if let Some(token) = token {
                                    ...
                                    let field_ty = case_payload_ty
                                        .get_field_type_at_index(i as u32)
                                        .ok_or_else(|| {
                                            anyhow::anyhow!("Binding field not found at {}", i)
                                        })?;
                                    if field_ty.is_pointer_type() {
                                        ...
                                        self.builder.build_store(var_ptr, loaded)?;
                                    ...
                                    if is_last && !is_void {
                                            ...
                                            &*stmt.borrow()
                                        {
                                            ...
                                            if let Some(value) = value {
                                                let value =
                                                    self.maybe_wrap_for_optional_return(value)?;
                                                self.emit_all_deinit_calls();
                                                self.emit_class_releases();
            ...
            Statement::Guard {
                    ...
                    } = &*left.borrow()
                {
                    ...
                    let check_bb = self.context.append_basic_block(fn_val, "guard_check");
                    let extra_check_bb =
                        self.context.append_basic_block(fn_val, "guard_extra_check");
                    let else_bb = self.context.append_basic_block(fn_val, "guard_else");
                    let exit_bb = self.context.append_basic_block(fn_val, "guard_exit");
                    self.builder.build_unconditional_branch(check_bb)?;
                    self.builder.position_at_end(check_bb);
                    ...
                    let enum_llvm_type = enum_types
                        ...
                        .ok_or_else(|| anyhow::anyhow!("Enum type '{}' not found", enum_name))?;
                    drop(enum_types);
                    let match_result = if let Some(raw_llvm_type) =
                        self.get_enum_raw_llvm_type(&enum_name)
                    {
                        let raw_ptr =
                            self.builder
                    ...
                    self.builder
                        .build_conditional_branch(match_result, extra_check_bb, else_bb)?;
                    ...
                    let extra_cond = self
                        ...
                        .into_int_value();
                    self.builder
                        .build_conditional_branch(extra_cond, exit_bb, else_bb)?;
                    self.builder.position_at_end(else_bb);
                    self.resolve_block_expression(else_body)?;
                    if else_bb.get_terminator().is_none() {
                        self.builder.build_unconditional_branch(exit_bb)?;
        ...
        match &*expr.borrow() {
                ...
                if let Some(getter_fn) = self.module.get_function(&getter_name) {
                    if let Some(ptr) = self.lookup_variable(&name.value) {
                        ...
                        if let Some(self_ptr) = class_self_ptr {
                            let result =
                                self.builder.build_call(getter_fn, &[self_ptr.into()], "")?;
                            match result.try_as_basic_value() {
                                inkwell::values::ValueKind::Basic(val) => Ok(Some(val)),
                                _ => Ok(None),
                            }
                    ...
                    if is_protocol_method {
                        ...
                        if let Some(protocol_name) = protocol_name {
                            ...
                            if let Some(fn_name) = fn_name
                                && let Some(f) = self.module.get_function(&fn_name)
                            {
                ...
                match operator {
                    BinaryOperator::Plus => {
                        ...
                        ) = (left_val, right_val)
                        {
                            ...
                            let gep =
                                unsafe { self.builder.build_gep(elem_ty, ptr, &[offset], "")? };
                            Ok(Some(gep.into()))
                        } else {
                    ...
                    BinaryOperator::Minus => {
                        ...
                        ) = (left_val, right_val)
                        {
                            ...
                            let gep =
                                unsafe { self.builder.build_gep(elem_ty, ptr, &[neg_offset], "")? };
                            Ok(Some(gep.into()))
                        } else {
                    ...
                    BinaryOperator::Equal => {
                        ...
                        ) = (left_val, right_val)
                        {
                            ...
                            let l_int =
                                self.builder
                                    .build_ptr_to_int(l, self.context.i64_type(), "")?;
                            let r_int = self.builder.build_ptr_to_int(
                                r_ptr,
                        ...
                        ) = (left_val, right_val)
                        {
                            let l_ptr = self
                                .builder
                                .build_extract_value(l, 0, "")?
                                .into_pointer_value();
                            let l_int = self.builder.build_ptr_to_int(
                                l_ptr,
                                self.context.i64_type(),
                                "",
                            )?;
                    ...
                    BinaryOperator::Less => {
                        ...
                        ) = (left_val, right_val)
                        {
                            ...
                            let l_int = self.to_i64_for_compare(left_val)?;
                            let r_int = self.to_i64_for_compare(right_val)?;
                            Ok(Some(
                                self.builder
                                    .build_int_compare(
                                        inkwell::IntPredicate::SLT,
                    ...
                    UnaryOperator::NotNullAssertation => {
                            ...
                            && !params.is_empty()
                        {
                            ...
                            let payload_val =
                                self.builder
                                    .build_load(some_payload_ty, some_payload_ptr, "")?;
                            let payload_val = if some_payload_ty.count_fields() == 1 {
                                let inner_ptr = self.builder.build_struct_gep(
                                    some_payload_ty,
                                    some_payload_ptr,
                                    0,
                                ...
                                let inner_ty = some_payload_ty
                                    .get_field_type_at_index(0)
                                    .ok_or_else(|| {
                                        anyhow::anyhow!("Some payload inner field not found")
                                    })?;
                                self.builder.build_load(inner_ty, inner_ptr, "")?
                            } else {
                                payload_val
                            };
                            self.builder.build_unconditional_branch(cont_bb)?;
                            self.builder.position_at_end(cont_bb);
                            let result_ty = if some_payload_ty.count_fields() == 1 {
                                some_payload_ty
                                    .get_field_type_at_index(0)
                                    .ok_or_else(|| {
            ...
            Expression::Assignment {
                ...
                let (var_ptr, current_val) = if let Some((name, ty_opt)) = var_info {
                    ...
                    if let Some(ty) = object_ty {
                        if let Type::Struct(struct_name, ..) = &*ty.borrow() {
                            ...
                            let class_name = self.normalize_class_name(class_name).into_owned();
                            let field_name = member.value.clone();
                ...
                } = &*left.borrow()
                {
                    ...
                    if let Some(ty) = &sub_ty
                        && let Type::Pointer(inner) | Type::NonNullPointer(inner) = &*ty.borrow()
                    {
                        ...
                        for p in sub_params {
                            let idx_val = self.resolve_expression(p.expression.clone())?.unwrap();
                            ...
                            let addr = unsafe {
                                self.builder.build_gep(element_type, ptr, &[idx_int], "")?
                            };
            ...
            Expression::If {
                ...
                } = &*condition.borrow()
                {
                    ...
                    if let (Some((Ok(alloca), llvm_ty)), Some(val)) = (&result_alloca, then_val) {
                        let val_to_store = if let (Some(t), BasicValueEnum::PointerValue(_)) =
                            (ty.as_ref(), &val)
                        {
            ...
            Expression::MemberAccess { object, member, .. } => {
                ...
                let struct_name = object_ty
                    ...
                    .or_else(|| {
                        ...
                        if self.struct_types.borrow().contains_key(&name) {
                            Some(name)
                        } else {
                            None
                        }
                ...
                if let Some(struct_name) = struct_name {
                    ...
                    let cross_module_getter_ty: Option<Rc<RefCell<Type>>> = 'search: {
                        ...
                        for prop in properties.iter() {
                            if prop.borrow().name().as_ref().ok() != Some(&field_name.to_string()) {
                                continue;
                            }
                ...
                let class_name = object_ty
                    ...
                    .map(|name| self.normalize_class_name(&name).into_owned());
                if let Some(class_name) = class_name {
                    let field_name = member.value.clone();
                    ...
                    let is_method = if let Some(scope) = self.program_scope.borrow().as_ref() {
                        if let Some(sym) = scope.borrow().get_symbol_deep(&class_name) {
                            ...
                            if let Symbol::Class { methods, .. } = &*binding {
                                methods.iter().any(|m| {
                                    if let Symbol::ClassMethod { name, .. } = &*m.borrow() {
                                        name == &field_name
                                    } else {
                                        false
                                    }
                    ...
                    if is_method {
                        anyhow::bail!(
                            "'{}' is a method of class '{}', not a stored field",
                            field_name,
                            class_name
                        );
                    }
                ...
                if let Some(ty) = &object_ty
                    && let Type::Enum(enum_name, weak_sym, ..) = &*ty.borrow()
                {
                    let member_name = member.value.clone();
                    let enum_name = enum_name.clone();

                    let enum_symbol = weak_sym.0.upgrade().or_else(|| {
                    ...
                    if let Some(symbol) = &enum_symbol {
                        let binding = symbol.borrow();
                        if let Symbol::Enum { properties, .. } = &*binding {
                            for prop in properties {
                                if prop.borrow().name().as_ref().ok() != Some(&member_name) {
                                    continue;
                                }
                                found_property = true;
                                let getter_base = format!("{}.{}.getter", enum_name, member_name);
                                let getter_name = self.mangled_fn_names.borrow()
                                    .get(&getter_base).cloned()
                                    .unwrap_or_else(|| self.mangle_fn_name(&getter_base, &[]));
                                if let Some(getter_fn) = self.module.get_function(&getter_name) {
                                    let param_count = getter_fn.count_params();
                                    let result = if param_count == 0 {
                                        self.builder.build_call(getter_fn, &[], "")?
                                    } else {
                                        let ptr_ty = self.context.ptr_type(inkwell::AddressSpace::from(0));
                                        let null_ptr = ptr_ty.const_null();
                                        self.builder.build_call(getter_fn, &[null_ptr.into()], "")?
                                    };
                                    let result_val = match result.try_as_basic_value() {
                                        inkwell::values::ValueKind::Basic(val) => val,
                                        _ => anyhow::bail!("Getter call did not return a value"),
                    ...
                    if found_property {
                        let ret_ty = expr.borrow().get_ty_ref()?.clone();
                        if let Some(ret_ty) = ret_ty {
                            let llvm_ty = self.resolve_type(ret_ty)?;
                            return Ok(Some(llvm_ty.const_zero()));
                        }
                        let ptr_ty = self.context.ptr_type(inkwell::AddressSpace::from(0));
                        return Ok(Some(ptr_ty.const_null().into()));
                    }

                    let case_name = member_name;
                                ...
                                self.try_declare_function_from_scope(&fn_name)
                            {
                                    ...
                                    || self.struct_types.borrow().contains_key(&fn_name)
                                {
                                    ...
                                    let class_decl = self
                                        ...
                                        .borrow()
                                        .as_ref()
                                        .and_then(|scope| scope.borrow().get_symbol_deep(&fn_name))
                                        .or_else(|| self.find_symbol_in_packages(&fn_name))
                                        .and_then(|sym| {
                                            let sym_b = sym.borrow();
                                ...
                                && let Type::Class(class_name, ..) = &*ty.borrow()
                            {
                                    ...
                                    self.get_vtable_slot_index(class_name, method_name)
                                {
                                    ...
                                    let fn_type = if let Some(declared_fn) =
                                        ...
                                        self.build_vtable_method_fn_type(class_name, method_name)?
                                    };
                    ...
                    _ => {
                        if let Expression::SelfType { ty, .. } = &*callee.borrow() {
                            if let Some(ty) = ty {
                                ...
                                if self.module.get_function(&mangled).is_none() {
                                    let is_class = matches!(&*ty.borrow(), Type::Class(..));
                                    let type_decl = self
                                        .program_scope
                                        .borrow()
                                        .as_ref()
            ...
            Expression::Case {
                ...
                let enum_name = if let Some(t) = enum_type.as_ref() {
                        ...
                        && let Type::Enum(name, _, _) = &*ty.borrow()
                    {
                        ...
                        anyhow::bail!("Case expression without enum type")
                    }
                };
                let case_idx = self.get_enum_case_index(&enum_name, &case_name.value)?;
                let enum_types = self.enum_types.borrow();
                ...
                if !bindings.is_empty() {
                    ...
                    if let Some(payload_type) = enum_payloads.get(&enum_name) {
                        let payload_union_ptr =
                            self.builder
                                .build_struct_gep(enum_llvm_type, subject_alloca, 1, "")?;
                        let case_payload_ptr = self.builder.build_struct_gep(
                            *payload_type,
                            payload_union_ptr,
                            case_idx as u32,
    ...
    fn find_symbol_in_packages(&self, name: &str) -> Option<Rc<RefCell<Symbol>>> {
        for pkg in self.packages.borrow().values() {
            for module in pkg.borrow().modules.values() {
                if let Some(scope) = &module.borrow().scope {
                    if let Some(sym) = scope.borrow().get_symbol_deep(name) {
                        return Some(sym);
                    }
                }
            }
    ...
    fn get_struct_field_type(
        ...
        let symbol_opt = self.program_scope.borrow().as_ref().and_then(|scope| {
            scope.borrow().get_symbol_deep(struct_name)
        }).or_else(|| self.find_symbol_in_packages(struct_name));
        if let Some(symbol) = symbol_opt {
            let binding = symbol.borrow();
            let (decl, properties) = match &*binding {
    ...
    fn infer_type_name_from_expr(&self, expr: &Rc<RefCell<Expression>>) -> Option<String> {
        ...
        if let (Some(object), Some(member_value)) = (object, member_value) {
            return self
                .infer_member_type_from_symbol_table(object, &member_value)
                .and_then(|prop_ty| {
